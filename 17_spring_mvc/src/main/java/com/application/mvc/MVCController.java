...
@RequestMapping("/basic") // localhost:8080/basic/main 
public class MVCController {
	
	// value에는 url 주소를 작성, method에는 요청방식을 작성한다. (method를 생략할 경우 GET, POST를 모두 처리한다.)
	// @RequestMapping(value="/main", method=RequestMethod.GET)
	// @GetMapping(value="/main")   => value는 생략 가능
	// @PostMapping("/main") // => method 방식 중에서 post만 받아옴
	@GetMapping("/main")
	public String main() {
		...
		/*
		 * # View Path
		   #spring.thymeleaf.prefix=classpath:/templates/  ← "main" 앞에
		   #spring.thymeleaf.suffix=.html                  ← "main" 뒤에
		 * 
		 * */
