	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			for (int i = 0; i < e.seat.length; i++) {
				if      (e.seat[i] == 0) System.out.print("[X] ");
				else if (e.seat[i] == 1) System.out.print("[O] ");
			}
			System.out.println();
			...
			if		(sel == 1) {		// 좌석예매
				...
				System.out.println("좌석 1 ~ 7번 선택 : ");
				int choice = scan.nextInt();
				
				if (1 > choice || choice > e.seat.length) {
					System.out.println("1 ~ 7번 좌석 중 선택해주세요.");
				}
				else {
					
					if (e.seat[choice - 1] != 1) {
						e.seat[choice - 1] = 1;
						e.money += 12000;
						e.bookCnt++;
					}
