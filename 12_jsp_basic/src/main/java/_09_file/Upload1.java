...
public class Upload1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MultipartRequest multipartRequest = new MultipartRequest(request, FileConfig.FILE_REPOSITORY_PATH , 1024 * 1024 * 100 , "utf-8", new DefaultFileRenamePolicy());
...
//		System.out.println("request : "          + request.getParameter("uploader")); // request : null
//		System.out.println("multipartRequest : " + multipartRequest.getParameter("uploader")); // multipartRequest : 왜될까?
		
		Enumeration<?> files = multipartRequest.getFileNames(); // <input type="file">인 모든 엘리먼트를 반환한다.
		...
		if (files.hasMoreElements()) { // 엘리먼트가 있으면
			...
			String contentType      = multipartRequest.getContentType(element);      // 파일의 타입을 반환(.txt, .exe 등)
