...
GROUP BY
		PRODUCT_CD
ORDER BY 
		TOTAL_ORDER_GOODS_QTY DESC #SUM(ORDER_GOODS_QTY) DESC
LIMIT
		3;
...
# (WHERE GROUP BY HAVING ORDER BY LIMIT 예시)2020년에 주문된 상품 중에서 사용자별로 주문상품별로 총 주문수량을 조회하되 총 주문수량이 5개 이상만 조회하고 주문수량이 가장많은 3개의 레코드만 조회하기.
SELECT
		MEMBER_ID,
        PRODUCT_CD,
        SUM(ORDER_GOODS_QTY) AS TOTAL_ORDER_GOODS_QTY
