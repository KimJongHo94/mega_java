...
# AOP ( Aspect-Oriented Programming ) 관점 지향 프로그래밍
	...
	- 프로젝트 개발 과정에서 핵심 기능 외에 추가적이고 다양한 부가(공통) 기능이 필요한다. (로깅 , 인증 , 보안 , 트랜잭션 , 테스트 , 오류대응 , 등등)
...
public class AOPAdvice {
	...
	// java.lang.IllegalArgumentException : execution 명시 오류
	
	@Pointcut("execution(public void com.application.aop.ex.*.work())") // 중복되는 명시자를 작성한다.
	public void abced() {
		// 내부적으로는 기능X
		// 포워딩 하는 기능으로 사용한다.
	}
	...
	// 메서드 호출 전
	//@Before("execution(public void com.application.aop.ex.*.work())")
	@Before("abced()")
	public void beforeWork() {
		System.out.println("(before) 출근한다.");
	}
	
	// 메서드 호출 후(예외(오류) 발생여부 관계없음)
	// @After("execution(* work())")
	// @After("execution(public void com.application.aop.ex.*.work())")
	@After("abced()")
	...
	// 메서드 호출 전후
	@Around("execution(public void com.application.aop.ex.*.getWorkingTime())")
	public void aroundGetWorkingTime(ProceedingJoinPoint pjp) {
		...
		System.out.println("업무 소요시간 : " + (endTime - startTime) / 1000.0 + "초");
	}
	
	// 메서드 호출 후(예외가 발생하지 않은 경우)
	@AfterReturning("execution(public void com.application.aop.ex.*.getInfo(..))") // getInfo(*,*), getInfo(String,int)
	public void afterReturningGetInfo(JoinPoint jp) { // JoinPoint객체를 통하여 메서드의 파라메타를 전달받을 수 있다.
	...
	@AfterThrowing("execution(public void com.application.aop.ex.Boss.getError())")
